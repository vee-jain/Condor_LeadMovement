...
hist(log1p(xdata$msd))

#' transforming by half the minimun non-zero value
xx = signif(0.5*sort(unique(xdata$msd))[2], 2)
hist(log(xdata$msd+xx))

#' this is equivalent to adding a small constant and log transforming
xdata$msd.2 <- xdata$msd + min(xdata$msd[xdata$msd>0])/2
hist(log(xdata$msd.2))
...
#### (6) theoretically identifiable random msdopes ----
xx.fe.re = fe.re.tab(fe.model = "msd~lead_level*day_prior + age",
...
plot(t.data$msd, t.data$n_points)

#' response
t.data$msd.2 <- t.data$msd + min(t.data$msd[t.data$msd>0])/2
range(t.data$msd.2)
t.data$log.msd.2 = log(t.data$msd.2)
hist(t.data$log.msd.2)

#### (8) running the model----
full.wac = lme4::lmer(log.msd.2~
