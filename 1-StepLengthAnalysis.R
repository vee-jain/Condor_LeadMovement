...
hist(log1p(xdata$m.sl))

#' transforming by half the minimun non-zero value
xx = signif(0.5*sort(unique(xdata$m.sl))[2], 2)
hist(log(xdata$m.sl+xx))

#' this is equivalent to adding a small constant and log transforming
xdata$m.sl.2 <- xdata$m.sl + min(xdata$m.sl[xdata$m.sl>0])/2
hist(log(xdata$m.sl.2))
...
xx.fe.re = fe.re.tab(fe.model = "m.sl~lead_level*day_prior + age",
...
#' include age as random slope in raw_id
xx.fe.re$summary[10]
...
#' we can’t include the random slope of the interaction between 
...
plot(t.data$m.sl, t.data$n_points)

#' response
t.data$m.sl.2 <- t.data$m.sl + min(t.data$m.sl[t.data$m.sl>0])/2
range(t.data$m.sl.2)
t.data$log.m.sl.2 = log(t.data$m.sl.2)
hist(log(t.data$m.sl.2))

#### (8) running the model----
full.wac = lme4::lmer(log.m.sl.2~
...
#' random intercepts and slopes there is a lower bound of zero
